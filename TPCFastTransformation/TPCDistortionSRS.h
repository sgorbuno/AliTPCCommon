...
/// \file  TPCDistortionSRS.h
/// \brief Definition of TPCDistortionSRS class
///
/// \author  Sergey Gorbunov <sergey.gorbunov@cern.ch>


#ifndef ALICE_ALITPCOMMON_TPCFASTTRANSFORMATION_TPCDISTORTIONSRS_H
#define ALICE_ALITPCOMMON_TPCFASTTRANSFORMATION_TPCDISTORTIONSRS_H
...
///
/// The TPCDistortionSRS class represents correction of nominal coordinates of TPC clusters
...
/// 
class TPCDistortionSRS :public FlatObject
{
 public:   
  ...
  TPCDistortionSRS(const TPCDistortionSRS& ) CON_DELETE;
 
  /// Assignment operator: disabled to avoid ambiguity. Use cloneTo[In/Ex]ternalBuffer() instead
  TPCDistortionSRS &operator=(const TPCDistortionSRS &)  CON_DELETE;

  /// Destructor
  ~TPCDistortionSRS() CON_DEFAULT;
  ...
  void cloneFromObject( const TPCDistortionSRS &obj, char *newFlatBufferPtr );
  void destroy();
  ...
  void setApproximationScenario( int scenarioIndex, const  SemiregularSpline2D3D &spline );
  ...
  const SemiregularSpline2D3D& getSpline( int slice, int row ) const;
 ...
 private:   
  ...
  std::unique_ptr<SemiregularSpline2D3D[]> mConstructionScenarios; ///< Temporary container for spline scenarios
  ...
  SemiregularSpline2D3D *mScenarioPtr; ///< Pointer to spline scenarios
...
inline int TPCDistortionSRS::convUVtoSUV( int slice, int row, float u, float v, float &su, float &sv ) const
{  
  //Brings the u coordinate from [0,uWidth] to [0, 1]
  //Brings the v coordinate from [0,tpcLengthSideA/C] to [0, 1]
  const RowInfo& rowInfo = getRowInfo( row );  
  su = (u-rowInfo.U0)*rowInfo.scaleUtoSU;

  //There are 18 slices on each side. So the first 18 are on the left, and the others on the right
  if( slice<18 ) sv = v*mScaleVtoSVsideA;
  else sv = v*mScaleVtoSVsideC;
  return 0;
}

inline int TPCDistortionSRS::convSUVtoUV( int slice, int row, float su, float sv, float &u, float &v ) const
{  
  //Brings the u coordinate from [0, 1] to [0, uWidth]
  //Brings the v coordinate from [0,tpcLengthSideA/C] to [0, 1]
  const RowInfo& rowInfo = getRowInfo( row );  
...
inline int TPCDistortionSRS::getDistortion( int slice, int row, float u, float v, float &dx, float &du, float &dv )
{
