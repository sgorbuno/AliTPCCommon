...
///
/// modified by Felix Lapp 11.09.2018
/// \author  Sergey Gorbunov <sergey.gorbunov@cern.ch>


#ifndef ALICE_ALITPCOMMON_TPCFASTTRANSFORMATION_SEMIREGULARSPLINE2D_H
#define ALICE_ALITPCOMMON_TPCFASTTRANSFORMATION_SEMIREGULARSPLINE2D_H
...
namespace ali_tpc_common {
  namespace tpc_fast_transformation {
	...
	void getSpline( const T *correctedData, float u, float v, T &x, T &y, T &z ) const;
      ...
      template <typename T>
	void getSplineUnoptimized( const T *correctedData, float u, float v, T &x, T &y, T &z ) const;
	...
	return reinterpret_cast<int*>( mFlatBufferPtr + mDataIndexMapOffset );
      ...
      RegularSpline1D mGridV; ///< grid for U axis
      //RegularSpline1D mGridU; ///< grid for V axis
      //RegularSpline1D *mSplineArray; ///< array of splines symbolizing "rows" of the grid
      int mNumberOfRows;
      int mNumberOfKnots;
	...
	{ // ==== low edge of V ====
	  ...
	  for( int iu=0; iu<nu; iu++ ){
	    ...
	    float u = gridU.indexToU(iu);

	    float x1=0, y1=0, z1=0, x2=0, y2=0, z2=0, x3=0, y3=0, z3=0;
	    getSpline(data, u, gridV.indexToU(1), x1, y1, z1);
	    getSpline(data, u, gridV.indexToU(2), x2, y2, z2);
	    getSpline(data, u, gridV.indexToU(3), x3, y3, z3);
	...
	{ // ==== high edge of V ====
	  ...
	  for( int iu=0; iu<nu; iu++ ){
	    ...
	    float u = getGridU(nv-1).indexToU(iu);

	    float x1=0, y1=0, z1=0, x2=0, y2=0, z2=0, x3=0, y3=0, z3=0;
	    getSpline(data, u, gridV.indexToU(nv-4), x1, y1, z1);
	    getSpline(data, u, gridV.indexToU(nv-3), x2, y2, z2);
	    getSpline(data, u, gridV.indexToU(nv-2), x3, y3, z3);
	...
	for(  int vi=0; vi<4; vi++, vxIndex+=3) {
	  ...
	  const int non = gridU.getNumberOfKnots();
	  dataVx[vxIndex+0] = RegularSpline1D::getSpline( ui, non, correctedData[dataOffset  ], correctedData[dataOffset+3], correctedData[dataOffset+6], correctedData[dataOffset+9 ], u);
	  dataVx[vxIndex+1] = RegularSpline1D::getSpline( ui, non, correctedData[dataOffset+1], correctedData[dataOffset+4], correctedData[dataOffset+7], correctedData[dataOffset+10], u);
	  dataVx[vxIndex+2] = RegularSpline1D::getSpline( ui, non, correctedData[dataOffset+2], correctedData[dataOffset+5], correctedData[dataOffset+8], correctedData[dataOffset+11], u);

	  //dataOffset0 += non;
	}

	//return results
	const int nonv = mGridV.getNumberOfKnots();
	x = RegularSpline1D::getSpline( iknotv, nonv, dataVx[0], dataVx[3], dataVx[6], dataVx[9 ], v );
	y = RegularSpline1D::getSpline( iknotv, nonv, dataVx[1], dataVx[4], dataVx[7], dataVx[10], v );
	z = RegularSpline1D::getSpline( iknotv, nonv, dataVx[2], dataVx[5], dataVx[8], dataVx[11], v );

      }

    template <typename T>
      inline void SemiregularSpline2D3D::getSplineUnoptimized( const T *correctedData, float u, float v, T &x, T &y, T &z ) const
      {
	// Get interpolated value for f(u,v) using data array correctedData[getNumberOfKnots()] with corrected edges

	// find the v indizes of the u-splines that are needed.
	int iv = mGridV.getKnotIndex(v);
	...
	for( int vi=0; vi<4; vi++) {
	  const RegularSpline1D &gridU = getGridU(vi+iv-1);
	  ...
	  const T *dataU0 = correctedData + getDataIndex(ui-1, iv+vi-1);
	  const T *dataU1 = dataU0 + 3;
	  const T *dataU2 = dataU0 + 6;
	  const T *dataU3 = dataU0 + 9;

	  // using spline interpolation we can find the x,y,z coordinates of one point along the u-spline
	  T resU[3];
	  for( int i=0; i<3; i++ ){
	    //interpolate the spline
  	    resU[i] = gridU.getSplineUnoptimized( ui, dataU0[i], dataU1[i], dataU2[i], dataU3[i], u);
          }

	  // save the results in dataVx array
	  dataVx[vi*3 +0] = resU[0];
	  dataVx[vi*3 +1] = resU[1];
	  dataVx[vi*3 +2] = resU[2];
        }

	// seperate the 4 points
	T *dataV0 = dataVx + 0;
	T *dataV1 = dataVx + 3;
	T *dataV2 = dataVx + 6;
	T *dataV3 = dataVx + 9;
	...
	T res[3];
	for( int i=0; i<3; i++ ){
	  // interpolate in v-coordinates
	  res[i] =mGridV.getSplineUnoptimized( iv, dataV0[i], dataV1[i], dataV2[i], dataV3[i], v );
        }
	...
	Vc::float_v resU = RegularSpline1D::getSpline( ui, gridU.getNumberOfKnots(), dt0, dt1, dt2, dt3, u);
	...
	dataU[vOffset+2] = resU[2];
      ...
      Vc::float_v res = RegularSpline1D::getSpline(vGridi, mGridV.getNumberOfKnots(), dataV0, dataV1, dataV2, dataV3, v);
      x = res[0];
      y = res[1];
      z = res[2];
