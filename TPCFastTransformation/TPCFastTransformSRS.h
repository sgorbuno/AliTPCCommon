...
/// \file  TPCFastTransformSRS.h
/// \brief Definition of TPCFastTransformSRS class
///
/// \author  Sergey Gorbunov <sergey.gorbunov@cern.ch>


#ifndef ALICE_ALITPCOMMON_TPCFASTTRANSFORMATION_TPCFASTTRANSFORMSRS_H
#define ALICE_ALITPCOMMON_TPCFASTTRANSFORMATION_TPCFASTTRANSFORMSRS_H
...
namespace ali_tpc_common {
  namespace tpc_fast_transformation {
    ...
    class TPCFastTransformSRS :public FlatObject
    {
    public:   
	...
	float padWidth; ///< width of pads [cm] 
      ...
      TPCFastTransformSRS(const TPCFastTransformSRS& ) CON_DELETE;
 
      /// Assignment operator: disabled to avoid ambiguity. Use cloneFromObject() instead
      TPCFastTransformSRS &operator=(const TPCFastTransformSRS &)  CON_DELETE;
   
      /// Destructor
      ~TPCFastTransformSRS() CON_DEFAULT;
      ...
      static constexpr size_t getClassAlignmentBytes() {return TPCDistortionSRS::getClassAlignmentBytes(); }
  
      /// Gives minimal alignment in bytes required for the flat buffer
      static constexpr size_t getBufferAlignmentBytes() {return TPCDistortionSRS::getBufferAlignmentBytes(); }
  

      /// Construction interface
  
      void cloneFromObject( const TPCFastTransformSRS &obj, char *newFlatBufferPtr );
      ...
      TPCDistortionSRS& getDistortionNonConst() { return mDistortion; }
    ...
    inline void TPCFastTransformSRS::setTPCrow( int iRow, float x, int nPads, float padWidth )
    {
      /// Initializes a TPC row  
    ...
    inline int TPCFastTransformSRS::convPadTimeToUV(int slice, int row, float pad, float time, float &u, float &v )
    {
    ...
    inline int TPCFastTransformSRS::convUVtoPadTime(int slice, int row, float u, float v, float &pad, float &time )
    {
      if ( slice<0 || slice>=NumberOfSlices || row<0 || row>=mNumberOfRows ) return -1;
    ...
    inline int TPCFastTransformSRS::convUVtoYZ(int slice, int row, float x, float u, float v, float &y, float &z )
    {
      if ( slice<0 || slice>=NumberOfSlices || row<0 || row>=mNumberOfRows ) return -1; 
    ...
    inline int TPCFastTransformSRS::convYZtoUV(int slice, int row, float x, float y, float z, float &u, float &v )
    {
      if ( slice<0 || slice>=NumberOfSlices || row<0 || row>=mNumberOfRows ) return -1;
    ...
    inline  int TPCFastTransformSRS::getTOFcorrection(int slice, int row, float x, float y, float z, float &dz )
    {
    ...
    inline int TPCFastTransformSRS::Transform( int slice, int row, float pad, float time, float &x, float &y, float &z )
    {
      /// _______________ The main method: cluster transformation _______________________  
      ///
      /// Transforms raw TPC coordinates to local XYZ withing a slice
