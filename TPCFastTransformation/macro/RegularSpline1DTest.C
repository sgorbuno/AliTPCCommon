...
#ifndef Spline1DTest_H
#define Spline1DTest_H
...
int RegularSpline1DTest()
{
  ...
  for( int i=0; i<nKnotsTot; i++){
    cout<<"knot "<<i<<": "<<spline.indexToU(i)<<endl;
  }

  const RegularSpline1D &gridU = spline;
  ...
  double diff=0;
  for( int i=0; i<nu; i++ ){
    double u = gridU.indexToU(i);
    double f0 = data0[i];
    knots->Fill( u, f0 );
    double fs = spline.getSplineUnoptimized( data, u );
    diff+=(fs-f0)*(fs-f0);
  }
  ...
  for( float u=0; u<=1.; u+=stepu ){
    iter++;
    float f = spline.getSplineUnoptimized( data, u );
    qaX->Fill( 1.e4*(f - F(u)) );
    n->Fill(u,F(u),f);
  }
